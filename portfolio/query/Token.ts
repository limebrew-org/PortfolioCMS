...
class TokenQuery {
    ...
    static async save(tokenEntity,routeType:String){
        return await tokenEntity
            .save()
            .then((tokenInfo)=>{
                if(routeType === "ADD") return ResponseStatusHandler.success_add(TokenQuery.schema)
                if(routeType === "UPDATE") return ResponseStatusHandler.success_update(TokenQuery.schema)
            })
            .catch((error)=>{
	...
	static async deleteById(id: String, profile_id: String) {}
